...
public class The_First_SceneTarget extends SimpleApplication {
    
    float[] scaleTextureCoordinates  = {1f,1f}; //尺度纹理坐标
    float[] init_rotation = {0f,0f,0f};  //初始旋转
    float[] first_cross_position = {0.0f,-0.01f,207f}; //第一个十字路�?????????????

    public static void main(String[] args) {
        The_First_SceneTarget app = new The_First_SceneTarget();
...
//        editor_add_Grand(physicsSpace,1000,1000);
//        editor_add_mainroad(0f,0f, false);
//        add_cross();
//         add_mainroad(4);
//         T_cross_init();
...
//         tree_init(5);
        ...
        //Declare the light of multiple directions
        ...
        //new added light
        DirectionalLight sun3 = new DirectionalLight();                         // light1
        sun1.setDirection(new Vector3f(-0.1f, -0.5f, 1.0f).normalizeLocal());
        rootNode.addLight(sun3);
        DirectionalLight sun4 = new DirectionalLight();                         // light2
        sun2.setDirection(new Vector3f(-1f, -0.1f, 1f).normalizeLocal());
        rootNode.addLight(sun4);
        
        DirectionalLight sun5 = new DirectionalLight();                         // light1
        sun1.setDirection(new Vector3f(0.1f, 0.7f, 1.0f).normalizeLocal());
        rootNode.addLight(sun5);
        DirectionalLight sun6 = new DirectionalLight();                         // light2
...
//user input start
        editor_speed_limit(-36.0f,-36.0f,true);
        editor_speed_limit(-72.0f,-18.0f,false);
        editor_speed_limit(-72.0f,-90.0f,true);
        editor_add_Grand(physicsSpace,660.0f,660.0f);
   ...
   public void T_cross_init(){
    ...
    public void editor_add_Grand(PhysicsSpace physicsSpace,float sizeX, float sizeZ){
        Texture grand_tex = tex_init("Scenes/Task/two/grass.png", true);
        float[] size = {sizeX, 0.01f, sizeZ};
        float[] num = {sizeX,sizeZ};
     ...
     public void editor_add_mainroad(float X, float Y, boolean direction){
        Texture road_tex = tex_init("Scenes/Task/two/ML2.jpg", true);
        float[] main_size = {9f,0.01f,9f};
        float[] rmian_scale = {22.22f/100*9,2f};
        float[] first_position = {X,0.01f,Y};
        if(direction){
            Spatial main_road = Spatial_init(main_size, rmian_scale, road_tex);
            add_sp(main_road, first_position, init_rotation);
        }else{
            float[] road_roatation = {0,1.57079f,0};
            Spatial main_road = Spatial_init(main_size, rmian_scale, road_tex);
            add_sp(main_road, first_position, road_roatation);
        }
     }
    
    
    
    public void editor_add_mainroad_origin(float X, float Y, boolean direction){
        Texture road_tex = tex_init("Scenes/Task/two/ML2.jpg", true);
        float[] main_size = {9f,0.01f,100f};
        float[] rmian_scale = {22.22f,2f};
        float[] first_position = {X,0.01f,Y};
        if(direction){
            Spatial main_road = Spatial_init(main_size, rmian_scale, road_tex);
            add_sp(main_road, first_position, init_rotation);
    ...
    public void editor_add_cross(float cross_sizeX,float cross_sizeY,float road_sizeX,float road_sizeY,float positionX,float positionY){
        float[] cross_size = {cross_sizeX,0.01f,cross_sizeY};
        float[] road_size = {road_sizeX,0.01f,road_sizeY};
        float[] position = {positionX,0.01f,positionY};
        ...
        float[] soild_line_size = {9f,0.01f,9f};
        ...
        for(int i = 0;i<4;i++){
            ...
            if(i % 2 == 0 ){
                ...
                float[] crossr_roatation = {0,-i*1.57079f,0};
                float[] road_position = {position[0]+(i-2)*cha, position[1], position[2]};
                float[] solid_position = {position[0]+(i-2)*cha2, position[1], position[2]};
                add_sp(cross_line, road_position, crossr_roatation);
                add_sp(solid_line, solid_position, crossr_roatation);
            }
        }
    ...
    public void editor_speed_limit(float x, float z, boolean direction){
        Spatial Speed = Load_model("Scenes/Task/speedLimit40/speedLimit40.scene", 2f, 2f, 2f);
        if (direction == true)
            Speed.rotate(0f, 3.1415926f, 0);
        else
            Speed.rotate(0f, -1.57059f, 0);
        add_sp(Speed, x, -1.1f, z, init_rotation);
